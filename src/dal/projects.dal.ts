...
export const updateProjectDetails = async (
  id: number,
  details: Partial<Omit<iProject, "id">> & { techIds?: number[] },
) => {
  await query("BEGIN");
  try {
    ...
    if (techIds !== undefined) {
      await query(`DELETE FROM project_technologies WHERE project_id = $1`, [
        id,
      ]);

      if (techIds.length > 0) {
        const placeholders = techIds
          .map((_, i) => `($1, $${i + 2})`)
          .join(", ");
...
export async function getProjectStats() {
  try {
    ...
    return {
      ...
      featured: parseInt(rows[0].featured),
    };
  } catch (error) {
