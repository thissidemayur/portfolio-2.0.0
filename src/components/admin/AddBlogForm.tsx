...
export function BlogForm({
  ...
  initialData?: iBlog; // Use your interface instead of any
  isEdit?: boolean;
}) {
  const router = useRouter();
console.log(`inidtal data: `,initialData)
  const {
    register,
    handleSubmit,
    ...
    defaultValues: initialData
      ? {
          ...initialData,
          // Ensure dates are not passed to form values directly
        }
      : {
          category: "TECHNICAL",
          is_featured: false,
  ...
  const onSubmit = async (data: BlogFormValues) => {
    // 2. Pass ID explicitly for updates
    const result =
      isEdit && initialData
  ...
  return (
    ...
    >
      ...
      <div className="lg:col-span-8 space-y-6">
        <section className="bg-[#0A0A0A] p-8 rounded-[2.5rem] border border-white/5 space-y-6">
          <InputWrapper label="Article_Title" error={errors.title?.message}>
            <input
              {...register("title")}
              className="w-full bg-black/40 border border-white/10 p-4 rounded-xl outline-none focus:border-[#00FF94]/50 transition-all"
            />
          </InputWrapper>
          ...
          <InputWrapper label="Summary" error={errors.summary?.message}>
            <textarea
              {...register("summary")}
              className="w-full bg-black/40 border border-white/10 p-4 rounded-xl outline-none h-24 resize-none transition-all"
            />
          </InputWrapper>
          ...
          <InputWrapper label="Content_Body" error={errors.content?.message}>
            <textarea
              {...register("content")}
              className="w-full bg-black/40 border border-white/10 p-6 rounded-2xl outline-none h-[500px] font-mono text-xs leading-relaxed transition-all"
            />
          </InputWrapper>
        </section>
      </div>
      ...
      <div className="lg:col-span-4 space-y-6">
        <section className="bg-[#0A0A0A] p-6 rounded-[2.5rem] border border-white/5 space-y-6">
          <div className="space-y-4">
            <InputWrapper label="Identifier_Slug" error={errors.slug?.message}>
              <input
                {...register("slug")}
                className="w-full bg-black/40 border border-white/10 p-3 rounded-lg outline-none font-mono text-xs"
              />
            </InputWrapper>
            ...
            >
              <select
                {...register("category")}
                className="w-full bg-black/40 border border-white/10 p-3 rounded-lg outline-none text-xs"
              >
          ...
          <button
            ...
            className="w-full py-5 bg-[#00FF94] text-black font-black uppercase text-xs tracking-widest rounded-2xl flex items-center justify-center gap-2 hover:shadow-[0_0_30px_rgba(0,255,148,0.2)] disabled:opacity-50 transition-all hover:cursor-pointer "
          >
            {isSubmitting ? (
...
// Reusable Helper Component for Errors
